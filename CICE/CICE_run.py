...
import CICE_helpers
from CICE import ConceptDiscovery, make_model
import argparse
...
def main(args):
  ...
  CICE_helpers.save_concepts(cd, discovered_concepts_dir)

  # Calculating CAVs and TCAV scores
  print('Calculating CAVS and TCAVS')
  cav_accuraciess = cd.cavs(min_acc=0.0)
  scores = cd.tcavs(test=args.stat_testing_filter)
  CICE_helpers.save_ace_report(cd, cav_accuraciess, scores,
                                 results_summaries_dir)
  # Plot examples of discovered concepts
  for bn in cd.bottlenecks:
    CICE_helpers.plot_concepts(cd, bn, 20, address=results_dir)
...
def parse_arguments(argv):
  ...
  parser.add_argument('--source_dir', type=str,
        ...
        concept folders are saved.''', default='SOURCE_DIR_foreground')
  parser.add_argument('--working_dir', type=str,
      help='Directory to save the results.', default='SAVE_DIR')
  parser.add_argument('--model_to_run', type=str,
                      help='The name of the pytorch model as in torch hub.', default='inception_v3')
  parser.add_argument('--feature_names', nargs='+', default=['Mixed_7c'],
  ...
  parser.add_argument('--target_class', type=str,
                      help='The name of the target class to be interpreted', default='timber_wolf')
