...
class ConceptDiscovery(object):
  ...
  def create_patches(self, method='slic', discovery_images=None,
    ...
    self.dataset, self.image_numbers, self.patches = \
      np.array(dataset, dtype=np.float32), np.array(image_numbers, dtype=np.int32), np.array(patches, dtype=np.float32)
  ...
  def _return_superpixels(self, img, method='slic',
    ...
    removing duplicates. If two patches have Jaccard similarity_results more than 0.5,
    ...
    if param_dict is None:
      param_dict = {}
    if method == 'slic':
      n_segmentss = param_dict.pop('n_segments', [15, 45, 60])
  ...
  def _patch_activations(self, imgs, bottleneck, bs=16, channel_mean=None, transform=None):
  ...
  def _get_gradients(self, acts, y, bottleneck_name, example=None):
    ...
    if 'features' in bottleneck_name:
      grads = -torch.autograd.grad(torch.sum(outputs[:, y[0]]), inputs)[0]
    else:
...
def make_model(settings):
  ...
  for name in settings.feature_names:
    if 'features[' in name:
      layer_index = int(name.split('[')[-1].split(']')[0])
      layer = model.features[layer_index]
    elif 'classifier[' in name:
      layer_index = int(name.split('[')[-1].split(']')[0])
      layer = model.classifier[layer_index]
    else:
